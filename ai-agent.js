...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const offTopicReply = text => {
    if (containsAny(text, ['pizza', 'pitsa', 'hampurilainen', 'kebab', 'sushi', 'ruoka', 'kahvi', 'olut'])) {
      return 'Pizzaa en valitettavasti osaa tilata 😄 Voin kuitenkin auttaa Nordhashin verkkosivuissa, Google-näkyvyydessä, chatboteissa ja automaatioissa.';
    }

    if (containsAny(text, ['mita kuuluu', 'miten menee', 'kuuluuko hyvaa'])) {
      return 'Hyvin, kiitos 😄 Olen täällä auttamassa Nordhashin palveluihin liittyvissä kysymyksissä.';
    }

    if (containsAny(text, ['kerro vitsi', 'kerro joku vitsi'])) {
      return 'Miksi verkkosivu meni terapiaan? Sillä oli liikaa latausaikoja 😄';
    }

    if (containsAny(text, ['lampotila', 'sataako', 'millainen keli', 'millainen ilma', 'saennuste'])) {
      return 'Säätä en näe tässä selainversiossa. Nordhashin palveluista osaan kuitenkin auttaa.';
    }

    if (containsAny(text, ['jaakiekko', 'jalkapallo', 'urheilutulos', 'kuka voittaa'])) {
      return 'Urheilutuloksia en osaa ennustaa 😄 Nordhashin palveluissa olen varmemmalla alueella.';
    }

    return '';
  };

  const isWebsiteFollowUp = text =>
    /\b([1-9]|1[0-9])\s*(?:sivu|sivua|alasivu|palvelu|palvelua)\b/.test(text)
    || containsAny(text, [
      'uusi sivu', 'uudet sivut', 'uusi verkkosivu', 'uudistus', 'uudistaa',
      'nykyinen sivu', 'vanha sivu', 'palvelua', 'alasivua', 'etusivu'
    ]);

  const isAutomationFollowUp = text => containsAny(text, [
    'tarjouspyynt', 'liidi', 'crm', 'raport', 'excel', 'sahkoposti',
    'lomake', 'kasittely', 'kopiointi', 'ajanvaraus', 'laskutus',
    'tiedonsiirto', 'manuaalinen', 'toistuva', 'kasityo'
  ]);

  const conversationalReply = text => {
    const exact = text.trim();

    if (
      ['ei', 'en', 'ei kiitos', 'en kiitos'].includes(exact)
      || containsAny(text, [
        'en halua mitaan', 'en tarvitse mitaan', 'ei kiinnosta',
        'en ole kiinnostunut', 'en halua ostaa', 'ei ole tarvetta',
        'jatan valiin', 'ei nyt', 'ehka joskus'
      ])
    ) {
      state.awaiting = '';
      state.lastService = '';
      return 'Selvä 😊 Ei tarvitse mitään. Voit vain kokeilla agenttia, ja jos mieleen tulee myöhemmin kysymys verkkosivuista, Google-näkyvyydestä tai automaatiosta, autan mielelläni.';
    }

    if (containsAny(text, ['testaan vaan', 'kokeilen vaan', 'testi', 'toimiiko tama', 'oletko botti', 'oletko oikea', 'kuka olet'])) {
      return 'Kyllä, olen Nordhashin palveluagentti — ja testi meni läpi 😄 Osaan vastata Nordhashin palveluista, hinnoista ja toteutustavoista. Kokeile vaikka: “Paljonko verkkosivut maksavat?”';
    }

    if (['joo', 'juu', 'kylla', 'kai', 'ehka'].includes(exact)) {
      if (state.awaiting === 'websiteScope') {
        return 'Hyvä. Kerro vielä lyhyesti: onko kyse uusista verkkosivuista vai nykyisten uudistuksesta, ja montako palvelua sivustolla pitäisi esitellä?';
      }
      if (state.awaiting === 'automationScope') {
        return 'Hyvä. Mikä toistuva työ vie eniten aikaa — esimerkiksi sähköpostien käsittely, tarjouspyynnöt, tietojen kopiointi vai raportointi?';
      }
      return 'Hyvä 😊 Mistä haluat aloittaa: verkkosivuista, Google-näkyvyydestä vai automaatiosta?';
    }

    if (containsAny(text, ['en tieda', 'vaikea sanoa', 'ihan sama', 'jotain vaan', 'mita vaan'])) {
      state.awaiting = '';
      return 'Ei haittaa. Aloitetaan tavoitteesta: haluaisitko mieluummin enemmän yhteydenottoja, paremman näkyvyyden Googlessa vai vähemmän käsin tehtävää työtä?';
    }

    if (containsAny(text, ['haha', 'heh', 'lol', 'hassu', 'vitsi'])) {
      return '😄 Hyvä heitto. Olen parhaimmillani yrityksen verkkosivuissa, SEO:ssa, chatboteissa ja automaatioissa — mutta pientä huumoria kestän kyllä.';
    }
  ...
  const fallbackReply = text => {
    ...
    state.awaiting = '';
    return 'Tuo meni vähän palvelualueeni ulkopuolelle 😄 Voin auttaa Nordhashin verkkosivuissa, Google-näkyvyydessä, chatboteissa, verkkokaupoissa ja automaatioissa.';
  };
  ...
  const reply = message => {
    ...
    if (!clean) {
      if (String(message || '').trim()) {
        addMessage('😄 Viesti tuli perille. Kysy minulta vaikka verkkosivuista, hinnoista, Google-näkyvyydestä tai automaatioista.', 'agent-bot');
      }
      return;
    }

    const conversational = conversationalReply(clean);
    if (conversational) {
      addMessage(conversational, 'agent-bot');
      return;
    }

    const offTopic = offTopicReply(clean);
    if (offTopic && !services.length) {
      state.awaiting = '';
      state.lastService = '';
      addMessage(offTopic, 'agent-bot');
      return;
    }

    /* Älä pakota uutta tai asiaan liittymätöntä viestiä aiempaan kysymykseen. */
    if (state.awaiting === 'websiteScope' && !isWebsiteFollowUp(clean) && !services.length) {
      state.awaiting = '';
    }
    if (state.awaiting === 'automationScope' && !isAutomationFollowUp(clean) && !services.length) {
      state.awaiting = '';
    }
    ...
    if (state.awaiting === 'automationScope' && isAutomationFollowUp(clean)) {
      addMessage(automationReply(clean), 'agent-bot');
      return;
    }

    if (state.awaiting === 'websiteScope' && isWebsiteFollowUp(clean)) {
